...
// Apple's standard corner radius is roughly 22.5% of the icon size
...
// Create a rounded rectangle path for the Apple "squircle" look
let roundedPath = NSBezierPath(roundedRect: bounds, xRadius: cornerRadius, yRadius: cornerRadius)
roundedPath.addClip() // Clip all subsequent drawing to this rounded shape
...
if let tiffData = image.tiffRepresentation,
   ...
   let pngData = bitmapImage.representation(using: .png, properties: [:]) {
    let url = URL(fileURLWithPath: "AppIcon_Rounded.png")
